...
// queryShopify constructs and sends a GraphQL query to Shopify to retrieve
// products that have tags matching both the city and category criteria.
func queryShopify(city string, category string) []shopifyProduct {
	...
	// For city filtering, if the code is "std:080" we use a combined tag.
	cityQuery := city
	if city == "std:080" {
		cityQuery = "Delhi OR 080"
	}

	// Build the GraphQL query with both city and category.
	gqlQuery := fmt.Sprintf(`{
		products(first: 10, query: "tag:%s AND tag:%s") {
			edges {
				node {
					id
					title
...
// queryShopifyForSelect calls queryShopify using both city and category parameters from the request.
func queryShopifyForSelect(req ONDCSelectRequest) []shopifyProduct {
	return queryShopify(req.Context.City, req.Message.Intent.Category.ID)
}

func queryShopifyWithCustomQuery(queryStr string) []shopifyProduct {
	shopifyURL := os.Getenv("SHOPIFY_URL")
	accessToken := os.Getenv("SHOPIFY_ACCESS_TOKEN")

	log.Printf("Starting Shopify custom query with URL: %s", shopifyURL)
	if shopifyURL == "" || accessToken == "" {
		log.Printf("Error: Missing Shopify configuration - URL: %s, Access Token: %s",
			shopifyURL,
			func() string {
	...
	log.Printf("Executing custom GraphQL query: %s", gqlQuery)
